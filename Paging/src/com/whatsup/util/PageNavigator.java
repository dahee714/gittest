...
public class PageNavigator{
	...
	private int pagePerGroup;		// 그룹 당 페이지 수(ex : 0번 그룹은 1~4 1번 그룹은 5~8 등 "페이지 수 = 4")
	private int currentPage;		// 현재 페이지(최근 글이 1부터 시작)
	private int totalRecordsCount;	// 전체 글 수
	private int totalPageCount;		// 전체 페이지 
	private int currentGroup;		// 현재 그릅(첫 그룹이 0번 그룹으로 시작)
	...
	public PageNavigator(int countPerPage, int pagePerGroup, int currentPage, int totalRecordsCount) {
				//페이지 당 글 수, 그룹당 페이지 수, 현재 페이지, 전체 글 수를 전달받음
